...
class CacheManager {
  ...
  keys(next) {
    let self = this;
    self.api.redis.clients.client.keys(this.redisPrefix + '*', (err, keys) => {
      next(err, keys);
    });
  }
  ...
  clear(callback) {
    ...
    self.keys((error, keys) => {
      ...
      _async2.default.parallel(jobs, error => {
  ...
  locks(next) {
    let self = this;
    self.api.redis.clients.client.keys(this.lockPrefix + '*', (err, keys) => {
      next(err, keys);
    });
  }
