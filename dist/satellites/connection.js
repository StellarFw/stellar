...
class Connections {
  ...
  constructor(api) {
    ...
    this.allowedVerbs = ['quit', 'exit', 'paramAdd', 'paramDelete', 'paramView', 'paramsView', 'paramsDelete', 'roomJoin', 'roomLeave', 'roomView', 'detailsView', 'say', 'event'];
    this.connections = {};
    this.api = api;
  }
  ...
  verbs(verb, words, callback = () => {}) {
    ...
    if (server && allowedVerbs.indexOf(verb) >= 0) {
      ...
      if (verb === 'quit' || verb === 'exit') {
        ...
        this.api.chatRoom.join(this.id, room).then(didHappen => {
        ...
        room = words[0];
        this.api.chatRoom.leave(this.id, room).then(didHappen => callback(null, didHappen)).catch(error => callback(error, false));
      } else if (verb === 'roomView') {
        // get requested room name
        room = words[0];

        if (this.rooms.indexOf(room) > -1) {
          this.api.chatRoom.status(room).then(status => callback(null, status)).catch(error => callback(error));
        } else {
          if (typeof callback === 'function') {
